...
for loss in loss_functions:
    ...
    for _ in range(2):
        ...
        classifierRNN.set_model(model_name="SimpleRNN", units=8, init="HeNormal", input_shape=[1, x_train.shape[1]],
                                activation=["sigmoid"], learning_rate=1e-4, optimizer="Adam",
                                loss=loss, epochs=30, batch_size=32)
        ...
        classifierCNN.set_model(model_name="CNN", units=8, init="HeNormal", input_shape=[x_train.shape[1]],
                                activation=["sigmoid", "sigmoid", "sigmoid"], learning_rate=1e-4, optimizer="Adam",
                                loss=loss, epochs=30, batch_size=32)
...
for i, metric in enumerate(data[0].keys()):
    ...
    for j in range(len(models)):
        loss_data = [data[j][metric][k] for k in range(len(loss_functions))]  # Get the data for the specific model
        x = np.arange(len(loss_functions))
        axs[row, col].bar(x + j * 0.2, loss_data, width=0.2,
                          label=models[j], color=colors[j])
        axs[row, col].set_title(metric)
        axs[row, col].set_ylim([0, 1])
        axs[row, col].set_xticks(x)
        axs[row, col].set_xticklabels(loss_functions)
        axs[row, col].legend()
