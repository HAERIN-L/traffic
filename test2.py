...
# Set up activation functions to try
...
for _ in range(2):
    ...
    for activation in activation_functions:
        classifierRNN.set_model(model_name="SimpleRNN", units=8, init="HeNormal", input_shape=[1, x_train.shape[1]],
                                activation=[activation], learning_rate=1e-4, optimizer="Adam",
    ...
    for activation in activation_functions:
        classifierCNN.set_model(model_name="CNN", units=8, init="HeNormal", input_shape=[x_train.shape[1]],
                                activation=[activation, activation, activation], learning_rate=1e-4, optimizer="Adam",
                                loss="binary_crossentropy", epochs=30, batch_size=32)
...
# Calculate the total number of activations used
num_activations = len(activation_functions)

# Set up the data for plotting
rnn_data = {'Accuracy': rnn_acc,
            'Precision': rnn_precision,
            'Recall': rnn_recall,
            'F1 Score': rnn_f1_score}

cnn_data = {'Accuracy': cnn_acc,
...
for i, metric in enumerate(data[0].keys()):
    ...
    for j in range(len(models)):
        activation_data = [data[j][metric][k] for k in range(num_activations)]  # Get the data for the specific model
        x = np.arange(num_activations)
        axs[row, col].bar(x + j * 0.2, activation_data, width=0.2,
                          label=models[j], color=colors[j])
        axs[row, col].set_title(metric)
        axs[row, col].set_ylim([0, 1])
        axs[row, col].set_xticks(x)
        axs[row, col].set_xticklabels(activation_functions)
        axs[row, col].legend()
...
results_df = pd.DataFrame({
    'Model': np.repeat(models, num_activations),
    'Activation Function': np.tile(activation_functions, len(models)),
    'Accuracy': np.concatenate([rnn_acc[:num_activations], cnn_acc[:num_activations]]),
    'Precision': np.concatenate([rnn_precision[:num_activations], cnn_precision[:num_activations]]),
    'Recall': np.concatenate([rnn_recall[:num_activations], cnn_recall[:num_activations]]),
    'F1 Score': np.concatenate([rnn_f1_score[:num_activations], cnn_f1_score[:num_activations]])
})
